...
int radarMovementSpeed = 20; // Max of 127, minimum of 1 (but realisticly, above 20)
int distances[3601]; // Array to store a distance every 1/10 degree. 1 short oversized to accomidate for the values the gyro outputs
int checkTime = 1; // Ms to check the ultrasonic sensor. Pretty sure below 20 is too fast
// for a cortex
bool includeRadarSetupDelay = false; // Includes a 6 second delay to clear the field before
...
void printValues() {
	...
	for(short degree = 1; degree < 3601; degree++)
	{
		short distance = distances[degree];
	//	if (distance == NULL || distance <= 0)
	//		writeDebugStreamLine("%i,0",degree);
	//	else
		wait1Msec(2);
			writeDebugStreamLine("%i,%i",degree,distance);
	}
...
void printNakedValues() {
	...
	for(short degree = 1; degree < 3600; degree++)
	{
		short distance = distances[degree];
		if (distance == NULL || distance < 0)
...
task printSensorValues() {
	...
	displayLCDNumber(1,5,getGyroValue());
}
...
task main()
{
	...
	while (true) {
		...
		if (thisValue == NULL || thisValue <= 0) { // If this value hasn't been properly set
			if (getGyroValue() > 3500){
				sprintf(str, "%i\n",getGyroValue());
				writeDebugStreamLine(str);
			}
			distances[getGyroValue()%3600] = getRadarValue();
			clearLCDLine(1);
